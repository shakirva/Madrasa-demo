...
export default function ParentHomeworkPage() {
  ...
  return (
    <DashboardLayout>
      <PageHeader title={t("parentPages", "homeworkTitle", lang)} subtitle={child.name} back />
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        <span className="flex items-center gap-1 text-xs text-gray-600"><span className="w-2.5 h-2.5 rounded-full bg-red-400 inline-block" />{t("parentPages", "pendingAction", lang)}</span>
        <span className="flex items-center gap-1 text-xs text-gray-600"><span className="w-2.5 h-2.5 rounded-full bg-yellow-400 inline-block" />{t("parentPages", "awaitingVerify", lang)}</span>
        <span className="flex items-center gap-1 text-xs text-gray-600"><span className="w-2.5 h-2.5 rounded-full bg-emerald-500 inline-block" />{t("parentPages", "verified", lang)}</span>
      </div>

      <SectionHeader title={t("parentPages", "hwList", lang)} className="mb-3" />
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{hw.subject} · {t("parentPages", "due", lang)} {hw.dueDate}</p>
                  </div>
                </div>
              ...
              {hw.type === "daily" && status === "red" && (
                ...
                >
                  ...
                  {saved === hw.id ? t("parentPages", "markedTeacherWill", lang) : t("parentPages", "markAsCompleted", lang)}
                </button>
              )}
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between text-xs text-gray-500 mb-1">
                      <span>{t("parentPages", "progress", lang)}</span>
                  ...
                  >
                    {saved === hw.id ? t("parentPages", "sentToTeacher", lang) : t("parentPages", "updateProgress", lang)}
                  </button>
                </div>
              )}
              ...
              {status === "yellow" && (
                <div className="flex items-center gap-2 bg-yellow-50 rounded-xl p-2.5">
                  ...
                  <p className="text-xs text-yellow-700">{t("parentPages", "submittedWaiting", lang)}</p>
                </div>
              )}
              ...
              {status === "green" && (
                <div className="flex items-center gap-2 bg-emerald-50 rounded-xl p-2.5">
                  ...
                  <p className="text-xs text-emerald-700">{t("parentPages", "verifiedByTeacher", lang)}</p>
                </div>
              )}
            </motion.div>
          );
